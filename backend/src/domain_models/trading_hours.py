...
class TradingHoursManagerV2:
    ...
    def is_trading_day(self, stock, date):
        info = self.info(stock.exchange)
        if info.get("can_trade_on_weekends", False):
            return True
        return date.weekday() <= 4

    def info(self, exchange):
        if exchange not in self.trading_info:
            raise HTTP400("Exchange for the given symbol not found.")

        return self.trading_info[exchange]


# Trading hours retrieved from
# https://www.thebalance.com/stock-market-hours-4773216#:~:text=Toronto%20Stock%20Exchange-,9%3A30%20a.m.%20to%204%20p.m.,30%20p.m.%20to%209%20p.m.&text=8%3A30%20a.m.%20to%203%20p.m.
# TODO fix times, and make start and end times be according to their own timezones...?
trading_hours_manager = TradingHoursManagerV2(
    dict(
        ASX=dict(start=time(10, 0), end=time(16, 0), timezone=timezone("Australia/Sydney")),
        NYSE=dict(start=time(9, 30), end=time(16, 0), timezone=timezone("America/New_York")),
        NASDAQ=dict(start=time(9, 30), end=time(16, 0), timezone=timezone("America/New_York")),
        LSE=dict(start=time(8, 0), end=time(16, 30), timezone=timezone("Europe/London")),
        XD00=dict(start=time(0, 0), end=time(6, 0), timezone=timezone("Australia/Sydney"), can_trade_on_weekends=True),
        XD06=dict(start=time(6, 0), end=time(12, 0), timezone=timezone("Australia/Sydney"), can_trade_on_weekends=True),
        XD12=dict(
            start=time(12, 0), end=time(18, 0), timezone=timezone("Australia/Sydney"), can_trade_on_weekends=True
        ),
        XD18=dict(
            start=time(18, 0), end=time(24, 0), timezone=timezone("Australia/Sydney"), can_trade_on_weekends=True
        ),
    )
)
